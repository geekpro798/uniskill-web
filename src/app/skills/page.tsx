...
export default function SkillsPage() {
    ...
    /* ── Token 注入逻辑 ──────────────────────────────────────────────────────
       已登录且有 rawToken（首次登录）→ 注入真实 token，显示 Copy 按钮
       已登录但无 rawToken（非首次）→ 占位符，提示去 Dashboard 查看
       未登录 → 占位符，按钮改为 Sign in to get Token
       ─────────────────────────────────────────────────────────────────────── */
                ...
                >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                        ...
                        {isLoading ? (
                            ...
                            // 未登录：登录后跳回 /skills 页面，注入真实 token
                            <motion.button
                    ...
                    <div className="code-block text-xs leading-relaxed font-mono flex items-start gap-2 overflow-x-auto">
                        ...
                        <span>
                            ...
                            <span className="text-slate-400"> -s -- </span>
                            {/* 根据登录状态与 rawToken 动态渲染 token 颜色与内容 */}
                            <span className={hasRealToken ? "text-cyan-400" : "text-slate-500 italic"}>{displayToken}</span>
                        </span>
                    </div>
